...
================================================================================
  ...
  // --------------------------------------------------------------------------
  // 11. Left Cyber Music Matrix Player (11 Audio Tracks Engine)
  // --------------------------------------------------------------------------
  function initCyberMusicMatrix() {
    const sidebar = document.getElementById('cyber-music-sidebar');
    const toggleBtn = document.getElementById('music-sidebar-toggle');
    const closeBtn = document.getElementById('music-sidebar-close');
    const trackItems = document.querySelectorAll('.music-track-item');
    const titleEl = document.getElementById('music-current-title');
    const statusBadge = document.getElementById('music-status-badge');
    ...
    const masterPlayBtn = document.getElementById('music-master-play');
    ...
    const volumeIcon = document.getElementById('music-vol-icon');
    ...
    function updateTrackUI() {
      tracks.forEach((t) => {
        const isCurrent = t.index === currentIdx;
        t.element.classList.toggle('active', isCurrent);
        t.element.classList.toggle('playing', isCurrent && isPlaying);

        const playIcon = t.element.querySelector('.track-play-icon i');
        if (playIcon) {
      ...
      if (statusBadge) {
        statusBadge.textContent = isPlaying ? 'STREAMING' : (currentIdx >= 0 ? 'PAUSED' : 'STOPPED');
    ...
    function playTrack(index) {
      ...
      if (currentIdx === index && audio.src) {
        if (isPlaying) {
          audio.pause();
          isPlaying = false;
          updateTrackUI();
          showToast(`AUDIO_STREAM: PAUSED "${track.name}"`, 'info');
        } else {
          audio.play().then(() => {
            isPlaying = true;
            updateTrackUI();
            showToast(`AUDIO_STREAM: RESUMED "${track.name}"`, 'success');
          }).catch(() => {});
        }
        return;
      }
      ...
      audio.play().then(() => {
        isPlaying = true;
        updateTrackUI();
        showToast(`AUDIO_STREAM: PLAYING "${track.name}"`, 'success');
      }).catch((err) => {
    ...
    function stopAudio() {
      ...
      showToast('AUDIO_STREAM: STOPPED', 'info');
    }

    tracks.forEach((t) => {
      t.element.addEventListener('click', () => {
        playTrack(t.index);
        playCyberBeep(720, 0.04);
      });
    });
    ...
    if (volumeSlider) {
      ...
      volumeSlider.addEventListener('input', (e) => {
        ...
        audio.volume = val;
        if (volumeIcon) {
          if (val === 0) volumeIcon.className = 'fas fa-volume-xmark text-magenta';
          else if (val < 0.5) volumeIcon.className = 'fas fa-volume-low text-cyan';
          else volumeIcon.className = 'fas fa-volume-high text-cyan';
        }
      });
    }
